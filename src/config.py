...
from .dataset.view_sampler import ViewSamplerCfg
...
def separate_dataset_cfg_wrappers(joined: dict) -> list[DatasetCfgWrapper]:
    ...
    @dataclass
    class DummyViewSampler:
        dummy: ViewSamplerCfg
    ...
    for k, v in joined.items():
        try:
            ...
            if k == "shoes":
                ...
                v_copy = OmegaConf.to_container(DictConfig(v), resolve=True)
                v_copy["view_sampler"] = load_typed_config(
                    DictConfig({"dummy": v_copy["view_sampler"]}),
                    DummyViewSampler,
                ).dummy
                shoes_cfg = from_dict(
                    DatasetShoesCfg,
